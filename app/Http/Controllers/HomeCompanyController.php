...
class HomeCompanyController extends Controller
{
    ...
    public function editarVuelo()
    {
        ...
        $flight = Flight::query()->select('id_flight','flights.id_price' ,'num_passengers', 'num_seats', 'num_check_in', 'flight_hours', 'departing', 'origin.name as origin', 'destination.name as destination', 'economic_price', 'business_price')
            ->join("cities as origin", "id_origin_city", "=", 'origin.id_city')
            ->join("cities as destination", "id_destination_city", "=", 'destination.id_city')
            ->join("flights_price", 'flights_price.id_price', '=', 'flights.id_price')
        ...
        return view('vuelosEditar', compact('flight', 'cities'));
    }

    public function vuelosActualizar()
    {
        $cityOrigin = City::query()->select('id_city')
            ->where('cities.name','=',request('origen'))
            ->get();
        $cityDestination = City::query()->select('id_city')
            ->where('cities.name','=',request('destino'))
            ->get();
        //dd(request('numSeats'));
        FlightsPrice::query()->where('id_price', '=', request('idPrice'))->update([
            'economic_price' => request('economic'),
            'business_price' => request('business'),
        ]);
        Flight::query()->where('id_flight', '=', request('idFlight'))->update([
            'num_passengers' => request('numPassengers'),
            'num_seats' => request('numSeats'),
            'departing' => request('fecha'),
            'id_origin_city' => $cityOrigin[0]['id_city'],
            'id_destination_city' => $cityDestination[0]['id_city'],
            'flight_hours' => (request('horas') . "h " . request('minutos') . "min"),
            'id_price' => request('idPrice'),
        ]);
